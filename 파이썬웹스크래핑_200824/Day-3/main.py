...
def add_to_dict(d, key='', val=''):
  if type(d) is not dict:
    print("You need to send a dictionary. You Sent: {}".format(type(d)))
  elif val == '':
    print("You need to send a word and a definition")
  else:
    if d.setdefault(key, val) == val:
...
def update_word(d, key='', val=''):
  if type(d) is not dict:
    ...
    if key not in d:
      print('{} is not on the dict. Can\'t update non-existing word.'.format(key))
    else:
      d[key] = val
...
def delete_from_dict(d, key=''):
  if type(d) is not dict:
    ...
    if key not in d:
      print('{} is not in this dict. Won\'t delete.'.format(key))
    else:
